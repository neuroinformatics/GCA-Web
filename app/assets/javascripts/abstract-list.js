require(['main'], function () {
  require(['lib/models', 'lib/tools', 'knockout', 'sammy', 'lib/offline'], function (
    models,
    tools,
    ko,
    Sammy,
      ...
      self.filteredAbsArr = ko.computed(function () {
        ...
        if (self.filter() && self.abstracts()) {
          filteredAbs = self.abstracts().filter(function (abstract) {
            ...
            return checkFields.filter((cFd) => cFd.toLowerCase().includes(self.filter().toLowerCase())).length > 0;
          });
        }
        return filteredAbs;
      });
