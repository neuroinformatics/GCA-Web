require(['main'], function () {
  require(['lib/models', 'lib/tools', 'lib/astate', 'knockout'], function (models, tools, astate, ko) {
    ...
    function adminAbstractsViewModel(confId) {
      ...
      self.selectableStates = ko.observableArray([
        'InPreparation',
        'Submitted',
        'InReview',
        'Accepted',
        ...
        init: function (element, valueAccessor) {
          $(element).click(function () {
            ...
            if (change) {
              ...
              valueAccessor().notifySubscribers(selected, 'arrayChange');
            }
          });
        },
      ...
      ko.bindingHandlers.bsDropdown = {
        init: function (element, valueAccessor) {
          $(element).click(function () {
            const abstract = ko.unwrap(valueAccessor());
            const state = $(element).text().trim();
            const dropdown = new bootstrap.Dropdown($(element).closest('.dropdown').find('.dropdown-toggle').get(0));
            dropdown.hide();
            self.setState(abstract, state, null);
          });
        },
      };
      ...
      self.beginStateChange = function (abstract) {
        ...
        const modal = new bootstrap.Modal(document.getElementById('state-dialog'));
        modal.show();
      };
      ...
      self.editorNoteCharactersLeft = ko.computed(function () {
        if (self.note()) {
