require(['main'], function () {
  require(['lib/models', 'lib/tools', 'lib/owned', 'knockout', 'dayjs', 'ko.sortable', 'datetimepicker'], function (
    models,
    tools,
    owned,
    ko,
    ...
    function adminConferenceViewModel(confId, accId) {
      ...
      ko.bindingHandlers.datetimepicker = {
        init: function (element, valueAccessor, allBindingsAccessor) {
          function onSelectDateHandler(ct, input) {
            var os = valueAccessor();
            var dateString = ct.toISOString();
            os(dateString);
          }

          var $el = $(element);
          var options = { onSelectDate: onSelectDateHandler, format: 'Y-m-d H:i O' };
        ...
        update: function (element, valueAccessor) {
          ...
          var current = $el.datetimepicker('getValue') || 0;
          if (value - current !== 0) {
            $el.val(dayjs(value).format('YYYY-MM-DD HH:mm ZZ'));
          }
        },
      };
      ...
      self.saveConference = function () {
        ...
        function addReplaceConfText(strType, text) {
          if (strType !== undefined && strType !== null && text !== undefined && text !== null) {
            for (var i = 0; i < self.conference().confTexts().length; i++) {
              if (
                self.conference().confTexts()[i] !== undefined &&
        ...
        if (
          !(self.oldShort === self.conference().short()) &&
          self.otherConfShorts().indexOf(self.conference().short()) >= 0
        ) {
