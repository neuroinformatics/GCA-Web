require(['main'], function () {
  require([
    ...
    function EditorViewModel(confId, abstrId) {
      ...
      // autosave label
      self.autosave = ko.observable({ text: 'Loading', css: 'btn-primary' });

      // required to set displayed modal header
      self.modalHeader = ko.observable('generalModalHeader');
      // required to set displayed modal body
      self.modalBody = ko.observable('generalModalBody');
      ...
      self.isAbstractSaved = ko.computed(function () {
        return self.abstract() && self.abstract().uuid;
      }, self);

      self.hasAbstractFigures = ko.computed(function () {
        return self.abstract() && self.abstract().figures().length > 0;
      }, self);

      self.latexInTitle = ko.computed(function () {
        var checkTitle = /.*\$.*\$.*/;
      ...
      self.showAbstractTextCharsLeft = ko.computed(function () {
      ...
      self.editorTextCharactersLeft = ko.computed(function () {
        var textCharLimit = $('#maxLenPrepText').text();
        if (self.editedAbstract() && self.editedAbstract().text() && textCharLimit !== undefined) {
      ...
      self.showAckCharsLeft = ko.computed(function () {
      ...
      self.editorAckCharactersLeft = ko.computed(function () {
        var ackCharLimit = $('#maxLenPrepAck').text();
        if (self.editedAbstract() && self.editedAbstract().acknowledgements() && ackCharLimit !== undefined) {
      ...
      self.validity = ko.computed(function () {
      ...
      self.showReason = ko.computed(function () {
        // dirty workaround part 2
        var _x = self.abstrGrChangeWatch();
        try {
          return self.conference().group === 'BCCN' && self.abstract().abstrTypes()[0].name === 'Talk or Poster';
        } catch (err) {
          return false;
        }
      }, self);
      ...
      self.requestAbstract = function (abstrId) {
        ...
        function success(obj) {
          ...
          self.editedAbstract(self.abstract());
          self.autosave({ text: 'Ok', css: 'btn-success' });
          self.fetchStateLog();
      ...
      self.doRemoveFigure = function (figure) {
        if (self.hasAbstractFigures()) {
          self.autosave({ text: 'Saving', css: 'btn-warning' });
      ...
      self.doSaveAbstract = function (abstract) {
        ...
        function successFig() {
          self.requestAbstract(self.abstract().uuid);
          self.autosave({ text: 'Ok', css: 'bg-success' });
        }
      ...
      self.fetchStateLog = function () {
        ...
        $.getJSON(logUrl, self.successStateLog).fail(function (jqxhr, textStatus, error) {
      ...
      self.doChangeState = function (toState) {
        ...
        if (toState === 'Submitted') {
          ...
          self.checkRemoveTopics(result.warnings);
          if (!result.ok()) {
            self.setError('Error', 'Unable to submit: ' + (result.hasErrors() ? result.errors[0] : result.warnings[0]));
            return;
          }
        }
      ...
      self.showButtonWithdraw = ko.computed(function () {
      ...
      self.showHelp = function () {
        ...
        if (!open) {
          if (self.abstract().state() === 'InRevision') {
            self.setInfo(
              'Abstract revision',
              'Please revise your abstract and re-submit your abstract by ' + 'clicking <b>Submit</b>',
            );
          } else {
            self.setWarning('Conference closed', 'Conference is closed and the abstract cannot be modified.');
          }
        }
        if (!self.isAbstractSaved()) {
          self.setInfo(
