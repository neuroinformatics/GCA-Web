 ...
 * @module {models}
 */
define(['lib/tools', 'lib/accessors', 'dayjs', 'knockout'], function (tools, acc, dayjs, ko) {
  'use strict';
      ...
      if (target.hasOwnProperty(prop)) {
        ...
        if (value !== null) {
          if (tools.type(target[prop]) !== 'function') {
            target[prop] = value;
          } else if (ko.isWritableObservable(target[prop])) {
            target[prop](value);
          }
        }
      }
  ...
  function Conference(
    ...
    self.formatCitation = function (abstract) {
      var year = dayjs(self.start).year();
      if (abstract.doi !== null) {
        return (
          abstract.formatAuthorsCitation() +
          ' (' +
          year +
          ') ' +
          abstract.title +
  ...
  function ObservableAbstract(
    ...
    self.indexedAuthors = ko.computed(function () {
      var indexed = [];
  ...
  function ObservableAccount(uuid, mail, fullName, ctime) {
    ...
    self.formatCtime = function () {
      if (self.ctime) {
        return dayjs(self.ctime()).format('YYYY/MM/DD');
      } else {
        return '';
      }
    };
  ...
  function Event(title, subtitle, start, end, date, location, authors, type, abstract) {
    ...
    self.title = title || null; // string
    self.subtitle = subtitle || null; // string
    self.start = start || null; // "09:00"
    self.end = end || null; // "14:00"
    self.date = date || null; // "2024-08-07"
    self.location = location || null; // string
    self.authors = authors || null; // string[]
    self.type = type || null;
    self.abstract = abstract || null; // url of abstract api
